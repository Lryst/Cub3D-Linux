...
{
	prt->conv == 1 ? ft_di_left(prt, ' ', args, count) : 0;
	prt->conv == 2 ? ft_di_left(prt, ' ', args, count) : 0;
	prt->conv == 3 ? ft_u_left(prt, ' ', args, count) : 0;
	prt->conv == 4 ? ft_x_l(prt, ' ', args, count) : 0;
	prt->conv == 5 ? ft_maj_x_l(prt, ' ', args, count) : 0;
	prt->conv == 6 ? ft_p_left(prt, ' ', args, count) : 0;
	prt->conv == 7 ? ft_s_l_space(prt, ' ', args, count) : 0;
	prt->conv == 8 ? ft_c_left(prt, ' ', args, count) : 0;
	prt->conv == 9 ? ft_percent_left(prt, ' ', count) : 0;
}
...
void	ft_width_p(t_flags *prt, size_t *count, va_list args)
{
	...
	{
		prt->conv == 1 ? ft_di_width_p(prt, args, count) : 0;
		prt->conv == 2 ? ft_di_width_p(prt, args, count) : 0;
